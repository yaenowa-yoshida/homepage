...
8. **`index.html` の継続性・程度の修飾を弱める**（コンテンツ・ユーザー承認後にマージ）
   - `index.html`「選ばれる理由」の「最新の活用知見を**常に**アップデートし**続けています**」は、
     必須ルール4が戒める程度・継続性の修飾（「〜し続けている」）に近い強さ。
   - 「継続的にアップデートしています」程度への緩和が候補。次回のコンテンツ改修時に併せて検討する。
