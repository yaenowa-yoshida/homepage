...
8. **継続性・程度の修飾を弱める**（コンテンツ・ユーザー承認後にマージ）
   - 必須ルール4が戒める程度・継続性の修飾（「〜し続けている」）に近い表現が3箇所残っている。
     - `index.html`「選ばれる理由」…「最新の活用知見を**常に**アップデートし**続けています**」
     - `about.html` 代表紹介 …「LT（ライトニングトーク）で発信を**続けています**」
     - `llms.txt` 代表者の活動 …「LT（ライトニングトーク）で発信を**継続**」
   - 「継続的にアップデートしています」「発信しています」程度への緩和が候補。
     緩和するなら**直近の活動時期を確認したうえで3箇所を同時に**（`business/ai.html` は
     2026-09-04 に「発信しています」へ緩和済みなので、揃えると4箇所が一貫する）。
9. **`services/nippo-slides.html` の `.text-link` 再定義を解消**（技術系・自動マージ可）
