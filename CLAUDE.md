...
5. **外部資料の引用** 🔴
   - **読んでいないURLを出典として貼らない。** Web検索が返すのは「リンク一覧」と
     「コーパス全体を要約したAI生成の文章」であり、**要約の記述がどのリンクに書いてあったかは
     保証されない**。両者を結びつけて出典として提示するのは出典の捏造にあたる。
     （2026-08-01 に実際に発生: 検索要約の数値を環境省PDFの出典つきで掲載したが、
     当該PDFに数値は存在しなかった）
   - 一次資料を読めていない場合は、**数値を出さない**か「検索で得られた記述」と明示する。
     「未検証です」という注記を添えて数値を載せるのは対策にならない（読者に届くのは数値のほう）。
   - **検索結果が矛盾したら、選ばずに報告する。** 複数の値が出てきたら版の違いを疑い、
     どれかを採用せずユーザーに判断を仰ぐ。
   - 数値を扱うページでは、**書く前に一次資料を依頼する**。実行環境によっては外部への
     HTTPS がネットワークポリシーで遮断され、検索要約しか見られないことがある
     （その場合 WebFetch は 403 を返す）。**PDFやスクリーンショットを渡してもらえば
     全文検証できる**ので、後追いではなく着手前に依頼すること。
   - 掲載する数値には**出典・統計期間・シナリオ・参照日**を添える。資料が古い場合は
     「同資料が示す推移は○年まで」等、現況を示すものではない旨を明記する
     （現在形で書くと現況と誤読される）。
   - 引用は要約であっても**原典の語をなるべく保つ**。程度・継続性の修飾語
     （「大幅に」「〜し続けている」等）を足さない。
...
- **レビューエージェントの「事実」に関する指摘は、マージのブロッカーとして扱う。** 🔴 でなくても、
  出典URLの妥当性・数値の裏付け・引用内容の一致に関する指摘は、解消するかユーザーの判断を
  仰ぐまでマージしない。「目視確認推奨」として報告に格下げして公開しないこと
  （2026-08-01 に実際に発生: 参照先ドメインが誤っているという指摘を報告扱いにしてマージし、
  誤った出典が公開された）。
