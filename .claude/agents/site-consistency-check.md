...
- **住所** `〒153-0064 東京都目黒区下目黒1丁目1番14号 コノトラビル7F`
  → **最新の一覧も件数も `CLAUDE.md`「所在地」の削除箇所を正とする**（ここに件数を写さない）。
    毎回 `grep -rn "コノトラ" . --exclude-dir=.git` を実行し、**一覧と実際のヒット箇所が
    一致するか突き合わせる**。増減があれば CLAUDE.md の一覧の更新漏れとして報告する。
    同じ一覧の写しを `scripts/check_site.py` の `ADDRESS_FILES` が持ち CI が検査するので、
    **CLAUDE.md・`ADDRESS_FILES`・grep の結果の3つが揃っているか**を見る。
    ずれていれば、どれが更新漏れなのかを明示して報告する。
